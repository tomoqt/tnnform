...
class TrueHigherOrderAttention(nn.Module):
    ...
    def true_higher_order_attention(self, projs):
        ...
        # Modify value projection to include key indices
        v = projs[-1]
        for _ in range(attention_order - 2):
            v = v.unsqueeze(2)  # Add dimensions for extra key indices
        v = v.expand(-1, -1, *([T] * (attention_order - 2)), -1, -1)  # Expand to match att's shape
