...
wandb_log_freq = 1  # log to wandb every 5 iterations
# data
dataset = 'openwebtext'
...
attention_order = 2  # Default to regular attention, can be overridden
# adamw optimizer
...
if init_from == 'scratch':
    ...
    ckpt_path = os.path.join(out_dir, 'ckpt_order_3.pt')
    checkpoint = torch.load(ckpt_path, map_location=device)
    checkpoint_model_args = checkpoint['model_args']
    # force these config attributes to be equal otherwise we can't even resume training
    # the rest of the attributes (e.g. dropout) can stay as desired from command line
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size',"attention_order"]:
